...
const useCheckApprove = (network: Network, approveTokenAddress: string, trancheMasterAddress: string) => {
...
export const useCheckApproveAll = (network: Network, approveTokenAddresses: string[], trancheMasterAddress: string) => {
  const { account } = useWeb3React();
  const signer = getSigner();
  const contracts = approveTokenAddresses.map((a, i) => getContract(ERC20.abi, a, network, signer));
